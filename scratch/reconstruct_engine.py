...
with open(log_path, 'r', encoding='utf-8') as f:
    for i, line in enumerate(f):
        if i >= 1337:
            # Skip steps from the current turn after the corrupt replace
            continue
        if 'engine.js' in line:
            try:
                obj = json.loads(line)
                # Check both content and output
...
if file_lines:
    ...
    with open(r"d:\Projects\workflow\js\engine.js.reconstructed", "w", encoding="utf-8") as out:
        out.write("\n".join(reconstructed))
    print("Saved to d:\\Projects\\workflow\\js\\engine.js.reconstructed")
